...
export class WalletStep extends LitElement {
                                        ...
                                        (['bsc', 'ethereum'].includes(this.selectedNetworkSymbol))
                                                ? html`
                                                    ...
                                                    <div @click=${() => this.selectWalletType('Injected')}
                                                         class="walletType custom injected"
                                                    >
                                                        <p>Injected</p>
                                                    ...
                                                    <div class="addressModal ${(this.showApproveAddressModal) ? 'show' : ''}">
                                                        ...
                                                        <div class="contentWrapper ${(this.showApproveAddressModalContent) ? 'show' : ''}">
                                                            <div class="content">
                                                                ...
                                                                <p class="text">
                                                                    Please confirm if you'd like to proceed using this wallet address to continue with the transaction.
                                                                </p>
    ...
    private createNewConnectorConfig() {
        console.log('createNewConnectorConfig')
    ...
    private async selectWalletType(type: WalletType) {
        ...
        // console.log('config', this.walletConnectorConfig)
        // if(account){
        ...
        const currentConnectionId = state.current;
        ...
        switch (type) {
            case "MetaMask":
                try {
                    ...
                    //     if(connection.connector.type === 'metaMask' && connection.connector['connect'] !== undefined){
                    ...
                    //     console.log(1)
                    //     const isAuth = await metaMaskConnector.isAuthorized();
                    ...
                    //     }else{
                    //         console.log(1.2)
                    //         connectResult = await metaMaskConnector.connect({
                    //             chainId: (this.selectedNetworkSymbol === 'bsc') ? bsc.id : mainnet.id,
                    ...
                    let metaMaskConnector = null;
                    for(let connection of connections.values()){
                        if((connection.connector.type === 'injected' || connection.connector.type === 'metaMask') && connection.connector.name === 'MetaMask' && connection.connector['connect'] !== undefined){
                            metaMaskConnector = connection.connector;
                        }
                    }
                    ...
                    if(!metaMaskConnector){
                        ...
                        metaMaskConnector = connectors?.find((item: any) => item.id === 'io.metamask' && item.type === 'injected');
                    }
                    ...
                    // const connectors = this.walletConnectorConfig.connectors;
                    // const metaMaskConnector = connectors?.find((item: any) => item.id === 'io.metamask' && item.type === 'injected');
            ...
            case "WalletConnect":
                try {
                    ...
                    if(walletConnectConnector){
                        console.log(1)
                        const isAuth = await walletConnectConnector.isAuthorized();
                        ...
                        if(isAuth && accounts.length > 0){
                            ...
                            console.log(1.2)
                            connectResult = await walletConnectConnector.connect({
                                chainId: (this.selectedNetworkSymbol === 'bsc') ? bsc.id : mainnet.id,
                                isReconnecting: false
                            });
                        }
                ...
                try {
                    ...
                    if(injectedConnector){
                        ...
                        if(isAuth && accounts.length > 0){
                            ...
                            console.log(1.2)
                            connectResult = await injectedConnector.connect({
                                chainId: (this.selectedNetworkSymbol === 'bsc') ? bsc.id : mainnet.id,
                                isReconnecting: false
                            });
                        }
    ...
    private async disconnectWallet(){
        ...
        if(this.walletType === 'MetaMask'){
            ...
            if(metaMaskConnectorMobile){
                await metaMaskConnectorMobile.disconnect();
            }

            if(metaMaskConnectorDesk){
