...
export class PaymentApp extends LitElement {
    ...
    private async createInvoice() {
        ...
        try {
            ...
            const result: Invoice | InvoiceCreateErrors = await response.json();

            if (response.status === 400) {
                throw new ValidationError<InvoiceCreateErrors>(result as InvoiceCreateErrors);
            }
            ...
            invoiceId = (result as Invoice).id;
            invoiceWS = ws.invoice(invoiceId);
        } catch (error) {
    ...
    private async getInvoice(invoiceId: string) {
        ...
        try {
            ...
            if (error instanceof HttpError) {
                errorTitle = 'Request Error';
                errorText = `An error occurred while receiving the invoice data. Please try again later.`;
            } else {
                errorTitle = 'Server Connection Error';
                errorText = `We were unable to connect to the server. Please check your internet connection and try again.`;
            }
    ...
    private async getTokens() {
        try {
            ...
            if (error instanceof HttpError) {
                errorTitle = 'Request Error';
                errorText = `An error occurred while receiving the token list. Please try again later.`;
            } else {
                errorTitle = 'Server Connection Error';
                errorText = `We were unable to connect to the server. Please check your internet connection and try again.`;
            }

            if (this.appStep === 'loading') {
                this.errorTitle = errorTitle;
                this.errorText = errorText;
                this.goToStep('error');
            } else {
